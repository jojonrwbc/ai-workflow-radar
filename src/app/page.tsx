...
import { benchmarkBoard, BenchmarkSummary, dailyNews, NewsItem } from "@/lib/feed-data";
...
function isReleaseItem(item: NewsItem): boolean {
  ...
  return /(release|update|launch|notes|version|changelog|neuankuendigung)/i.test(
    haystack,
  );
}
...
function getVisibleItems(mode: ViewMode, items: NewsItem[]): NewsItem[] {
  ...
  if (mode === "builder") {
    return items.filter(isBuilderItem);
  }

  if (mode === "releases") {
    return items.filter(isReleaseItem);
  }
...
export default function Home() {
  ...
  const [items, setItems] = useState<NewsItem[]>(dailyNews);
  const [benchmarks, setBenchmarks] = useState<BenchmarkSummary[]>(benchmarkBoard);
  ...
  async function refreshFeed(options?: {
    ...
    try {
      ...
      // Keep seeded fallback data when API fetch fails.
    } finally {
      if (showRefreshingLabel) {
  ...
  useEffect(() => {
    ...
    const timerId = window.setTimeout(() => {
      refreshFeed({ signal: controller.signal, showRefreshingLabel: true });
    }, 0);
  ...
  return (
    ...
    >
      <header className="rounded-2xl border border-radar-stroke bg-radar-panel px-5 py-6 sm:px-7">
        <div className="flex items-start justify-between gap-3">
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-radar-mint">
            AI Workflow Radar
          </p>
          <ThemeToggle />
        </div>
        <h1 className="mt-3 max-w-3xl font-[var(--font-display)] text-4xl leading-[1.05] text-radar-ink sm:text-5xl">
          Daily AI feed, reduziert auf das, was wirklich nutzbar ist.
        </h1>
        <p className="mt-4 max-w-2xl text-sm leading-6 text-radar-muted">
          Klare Streams fuer Builder-News, allgemeine KI-Welt und neue Releases.
        </p>
        {isRefreshing ? (
          <p className="mt-3 text-xs text-radar-muted">Aktualisiere Feed ...</p>
        ) : null}
      </header>
      ...
      >
        <div className="flex h-14 items-center justify-center text-xs text-radar-muted">
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`rounded-full border px-3 py-2 text-sm font-medium transition ${
              mode === tab.id
                ? "border-radar-mint bg-radar-mint text-white"
                : "border-radar-stroke bg-radar-panel text-radar-ink hover:bg-radar-soft"
            }`}
          >
            {tab.label}
          </button>
      ...
      <section className="mt-4 lg:hidden">
        ...
        <div className="mt-2 flex gap-3 overflow-x-auto pb-1">
          {storyItems.map((item, index) => (
            ...
            >
              <div className="relative h-24 w-24 overflow-hidden rounded-full border border-radar-stroke">
              ...
              <p className="mt-1 line-clamp-2 text-[11px] font-medium text-radar-ink">
                {item.title}
              </p>
            </button>
          ))}
      ...
      <main className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
        <section className="space-y-4">
          {visibleItems.map((item) => (
            <article
              key={item.id}
              className="overflow-hidden rounded-2xl border border-radar-stroke bg-radar-panel"
            >
              <div className="relative h-44 w-full">
                ...
                />
                <div className="absolute left-3 top-3 z-20 rounded-full border border-radar-stroke bg-radar-panel/90 px-2.5 py-1 text-xs font-medium text-radar-ink">
                  {item.category}
                </div>
              </div>

              <div className="space-y-3 p-4 sm:p-5">
                <div className="flex items-center justify-between gap-3 text-xs text-radar-muted">
                  <span>{formatDate(item.publishedAt)}</span>
                  <span className="rounded-full bg-radar-soft px-2.5 py-1 font-medium text-radar-ink">
                    Score {item.score}
                  </span>
                </div>

                <h2 className="text-xl font-semibold leading-snug text-radar-ink">
                  {item.title}
                </h2>
                <p className="text-sm leading-6 text-radar-muted">{item.lead}</p>

                <div className="rounded-xl bg-radar-soft px-3 py-2.5">
                  <p className="text-xs font-semibold uppercase tracking-[0.08em] text-radar-muted">
                    Warum relevant
                  </p>
                  <p className="mt-1 text-sm text-radar-ink">{item.whyItMatters}</p>
                </div>

                {item.benchmark ? (
                  <div className="flex items-center justify-between rounded-xl border border-radar-stroke px-3 py-2 text-sm">
                    <div>
                      <p className="font-medium text-radar-ink">{item.benchmark.label}</p>
                      <p className="text-radar-muted">{item.benchmark.value}</p>
                    </div>
                    <span className="rounded-full bg-radar-soft px-2.5 py-1 text-xs font-medium text-radar-ink">
                      {item.benchmark.delta}
                    </span>
                  </div>
                ) : null}

                <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-radar-muted">
                  <span>{item.sourceName}</span>
                  <a
                    href={item.sourceUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="font-medium text-radar-ink hover:underline"
                  >
                    Quelle ansehen
                  </a>
                </div>
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  <Link
                    href={`/article/${item.id}`}
                    className="rounded-full border border-radar-mint bg-radar-mint px-3 py-2 text-center text-sm font-medium text-white transition hover:opacity-90"
                  >
                    Artikel lesen
                  </Link>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="rounded-full border border-radar-stroke bg-radar-panel px-3 py-2 text-center text-sm font-medium text-radar-ink transition hover:bg-radar-soft"
                  >
                    Originalquelle
                  </a>
                </div>
        ...
        <aside className="space-y-4 lg:sticky lg:top-4 lg:h-fit">
          <section className="rounded-2xl border border-radar-stroke bg-radar-panel p-4">
            <h3 className="text-sm font-semibold uppercase tracking-[0.08em] text-radar-muted">
              Tagesbenchmarks
            </h3>
              ...
              {benchmarks.map((entry) => (
                <div key={entry.label}>
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-medium text-radar-ink">{entry.label}</span>
                    <span className="text-radar-muted">{entry.value}</span>
                  </div>
                  <div className="mt-1 h-1.5 rounded-full bg-radar-soft">
                    <div
                      className="h-full rounded-full bg-radar-mint"
                      style={{ width: `${Math.min(entry.score, 100)}%` }}
                    />
                  </div>
                  <p className="mt-1 text-xs text-radar-muted">Delta {entry.delta}</p>
                </div>
              ))}
            </div>
          </section>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-20 border-t border-radar-stroke bg-radar-panel/95 px-3 py-2 backdrop-blur lg:hidden">
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`rounded-full border px-2 py-2 text-xs font-medium ${
                mode === tab.id
                  ? "border-radar-mint bg-radar-mint text-white"
                  : "border-radar-stroke bg-radar-soft text-radar-ink"
              }`}
            >
              {tab.label}
            </button>
      ...
      {storyOpen && currentStory ? (
        ...
        >
          <div className="relative h-full w-full">
            {isLocalProxyImage(currentStory.imagePath) ? (
              <img
